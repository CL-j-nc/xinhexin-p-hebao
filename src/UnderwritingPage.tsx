...
const UnderwritingPage: React.FC = () => {
    ...
    const handleCeremonySubmit = async () => {
        ...
        try {
            ...
            const jsonDec = (await resDec.json()) as { success: boolean; error?: string; authCode?: string };
            ...
            if (acceptance === 'ACCEPT') {
                ...
                // Display auth code for the underwriter to provide to the customer
                if (jsonDec.authCode) {
                    alert(`核保通过！客户认证验证码：${jsonDec.authCode}\n\n请将此验证码告知客户，客户需在认证页面输入手机号和此验证码完成认证。`);
                } else {
                    alert("核保完成！保单已签发。");
                }
            } else {
