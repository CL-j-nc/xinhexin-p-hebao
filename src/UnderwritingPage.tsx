    ...
    const selectProposal = async (id: string) => {
        ...
        try {
            ...
            const merged = data.coverage.map((c: any) => ({
                ...
                base_premium: c.base_premium || (c.sum_insured || 0) * 0.01, // Default base if missing
                rate: c.rate || 1.0, // Default rate to 1.0 (100%) if using base premium
                premium: (c.base_premium || (c.sum_insured || 0) * 0.01) * (c.rate || 1.0),
            ...
            const merged = pd.coverages.filter((c: any) => c.selected !== false).map((c: any) => ({
                ...
                base_premium: (c.amount || 0) * 0.01, // approximate base
                rate: 1.0,
                premium: ((c.amount || 0) * 0.01) * 1.0,
            }));
            setCoverages(merged);
...
const updateCoverage = useCallback((index: number, field: keyof CoverageItem, value: any) => {
    setCoverages(prev => {
        ...
        const base = next[index].base_premium || 0;
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button
                                                        onClick={async () => {
                                                            ...
                                                            if (!confirm(`确认生成金额为 ¥${totalPremium} 的收款链接吗？\n收款账号: H15348806977`)) return;

                                                            try {
                                                                // Call Backend Proxy which calls Worker
                                                                ...
                                                                if (json.success && json.paymentLink) {
